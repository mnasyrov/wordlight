...
namespace WordLight
{
	public class TextViewWindow : NativeWindow, IDisposable
	{
		...
		private void DrawSearchMarks(Graphics g, User32.RECT updateRect)
		{
			...
			{
				int x = Math.Min(clipRect.Left, updateRect.Left) - 1;
				int y = updateRect.Top - 1;
				int width = updateRect.Right - updateRect.Left + 2;
				int height = updateRect.Bottom - updateRect.Top + 2;
				clipRect = new Rectangle(x, y, width, height);
			}

			Rectangle[] searchMarks = _searchMarks.GetRectanglesForVisibleMarks(_searchStart, _searchEnd, clipRect, _view, _lineHeight, _buffer);
			Rectangle[] freezed1 = _freezeMarks1.GetRectanglesForVisibleMarks(_searchStart, _searchEnd, clipRect, _view, _lineHeight, _buffer);
			Rectangle[] freezed2 = _freezeMarks2.GetRectanglesForVisibleMarks(_searchStart, _searchEnd, clipRect, _view, _lineHeight, _buffer);
			Rectangle[] freezed3 = _freezeMarks3.GetRectanglesForVisibleMarks(_searchStart, _searchEnd, clipRect, _view, _lineHeight, _buffer);
		...
		private void InvalidateVisibleMarks(MarkCollection marks)
		{
			...
			Rectangle[] rectList = marks.GetRectanglesForVisibleMarks(_searchStart, _searchEnd, maxClipBounds, _view, _lineHeight, _buffer);
		...
		private void ScrollChangedHandler(object sender, ViewScrollChangedEventArgs e)
		{
			...
			if (e.ScrollInfo.IsVertical)
			{
				...
				_searchStart = _buffer.GetPositionOfLineIndex(_viewRange.iStartLine, _viewRange.iStartIndex);
				_searchEnd = _buffer.GetPositionOfLineIndex(_viewRange.iEndLine, _viewRange.iEndIndex);
			}
		}

		private void StreamTextChangedHandler(object sender, StreamTextChangedEventArgs e)
		{
			//_freezeMarks1.ReplaceMarks(_freezeSearch1.SearchOccurrences(_freezeText1, _searchStart, _searchEnd));
			//_freezeMarks2.ReplaceMarks(_freezeSearch2.SearchOccurrences(_freezeText2, _searchStart, _searchEnd));
			//_freezeMarks3.ReplaceMarks(_freezeSearch3.SearchOccurrences(_freezeText3, _searchStart, _searchEnd));
		...
		private void SelectionChanged(string text)
		{
			...
			{
				var marks = _search.SearchOccurrences(_selectedText, _searchStart, _searchEnd);
